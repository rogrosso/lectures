...
import { bSplineTessellation, computeBSplinePatches, boehmPatches } from 'tpSurfaces'
...
const ground = new THREE.Mesh(
    ...
    new THREE.MeshLambertMaterial( { color: 0xa0adaf, side: THREE.DoubleSide } )
)
...
folderVis.add(guiProps.visualization,'patches').onChange( v => {
    if (v === true ) {
        ...
        wMaterial.forEach( (c,i) => c.color = colorMap[i].clone().multiplyScalar(0.3))
    }
    else {
        mMaterial.forEach( (c,i) => c.color = defaultColor )
        wMaterial.forEach( (c,i) => c.color = defaultColor.clone().multiplyScalar(0.3))
    }
})
folderVis.add(guiProps.visualization, 'controlNet').name('control net').onChange( v => {
    if (v === true) {
        lGroup.children.forEach( l => { l.material.visible = true })
    }
    else {
        lGroup.children.forEach( l => { l.material.visible = false })
    }
})
folderGeo.add(guiProps.geometry, 'level', 2, 5, 1).name('sampling level').onFinishChange( updateGeometry )
folderGeo.add(guiProps.geometry, 'subdivision', 0,maxSubdivision,1).name('subdivision').onChange( updateGeometry )
const geoState = {
...
for (let level = 1; level <= maxSubdivision; level++) {
    patches.push( boehmPatches( patches[level - 1], ns[level-1], nt[level-1] ) )
}
...
for (let i = 0; i < nrPatches; i++) {
    const c = colors()
    colorMap.push(new THREE.Color("rgb(" + c[0] + "," + c[1] + "," + c[2] + ")"))
    mMaterial.push(
...
function updateGeometry() {
    if (geoState.level !== guiProps.geometry.level || geoState.subdivision !== guiProps.geometry.subdivision) {
...
function updateTeapot(mGroup, wGroup, mMaterial, wMaterial, patches, level) {
    ...
    Object.entries(mesh).forEach(([key, patch], index) => {
        ...
        geometry.setAttribute(
            "position",
            new THREE.Float32BufferAttribute(vertices, 3).onUpload(disposeArray)
        )
