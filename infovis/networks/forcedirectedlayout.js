...
function jiggle() { return (randJiggle() - 0.5) * 1e-4 }
...
function collisionForce(k, beta, nodes, i, j, disp) {
...
const colors = ['#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#D2691E','#b15928']
const colorScale = d3.scaleOrdinal()
    .domain(Array.from(gSet).sort())
    .range(colors)
    
const beta = 0.2
const linkG = netwG.append("g")
    .attr("stroke", "#999")
...
function animate() {
    ...
    step(K, Kc, Kg, cR, nodes, edges, bbox, disp, alpha) 
    alpha = alpha - 0.005 * Math.exp(1/iter) // alphaDecay
...
function initNetwork(data, width, height) {
    ...
    for (let [key, value] of eMap) {
        edges.push({source: value.source, target: value.target, value: value.value, key: key })
    }
    eMap.clear()
    ...
    nodes.forEach(n => { n.c = n.degree })
    // compute radius depending on centrality
    const minC = nodes.reduce(function(prev, curr) { return prev.c < curr.c ? prev : curr })
    const maxC = nodes.reduce(function(prev, curr) { return prev.c > curr.c ? prev : curr })
    const lerp = (domain, range, u) => {
        return range[0] + (u - domain[0]) / (domain[1] - domain[0]) * (range[1] - range[0])
    }
