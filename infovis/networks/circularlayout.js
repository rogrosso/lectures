...
function draw(nodes, edges, group) {
    ...
    // nodes color
    const colorScale = d3.scaleOrdinal(d3.schemePaired).domain(domain.sort())
    ...
    const eGroup = g
        ...
        .data(edges)
        .join('path')
                .attr('d', d => cubicBezier(1, lg, nodes[d.source], nodes[d.target]))
                .attr("stroke", d => d3.hsl(colorScale(nodes[d.source].group)).darker(1))
                .attr("stroke-width", 2)
                .attr("fill", "none")
                .attr("opacity", 0.2)
    const nGroup = g
        ...
        .join(
            (enter) => {
                enter
                    ...
                    .attr("stroke", d => d3.hsl(colorScale(d.group)).brighter(1))
                    .attr("name", (d) => d.name)
...
function initNetwork(netw) {
    ...
    nodes.forEach(n => { n.c = nodeScale(n.degree) })
}
    ...
    t2.selectAll('path')
        .attrTween('d', (d, i, a) => {
            return function(t) {
                if (t < 1/2 ) {
                    const u = 2 * t
                    return cubicBezier(1-u, lg, nodes[d.source], nodes[d.target])
                } else {
                    const u = 2 * (t - 1/2)
                    const lk = d3.line()
                        .x(d => d.x)
